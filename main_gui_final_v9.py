...
def write_grade_to_exif(image_path, score):
    """Writes the grade to the image's EXIF data."""
    try:
        command = ['exiftool', '-overwrite_original', f'-ImageDescription=PhotoGrade: {score:.2f}', image_path]
        subprocess.run(command, check=True, capture_output=True, text=True)
    except Exception as e:
        print(f"Could not write grade to {os.path.basename(image_path)} using ExifTool: {e}")
...
def add_5_star_rating_exiftool(image_path):
    ...
    try:
        ...
        print(f"Could not write 5-star rating to {os.path.basename(image_path)} using ExifTool: {e}")
