...
def analyze_facial_features(image_path, teeth_sensitivity):
    ...
    try:
        ...
        if has_smile:
            ...
            if white_percentage > teeth_sensitivity:
                teeth_showing = 1
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.teeth_label = ctk.CTkLabel(self.slider_frame, text="Teeth Sensitivity:")
        self.teeth_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.teeth_slider = ctk.CTkSlider(self.slider_frame, from_=1, to=10, number_of_steps=9, command=self.update_teeth_label)
        self.teeth_slider.set(3) # Default to 3%
        self.teeth_slider.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        self.teeth_value_label = ctk.CTkLabel(self.slider_frame, text="3%")
        self.teeth_value_label.grid(row=1, column=2, padx=10, pady=5, sticky="e")
        ...
        self.teeth_sensitivity = 3.0

    def update_headsize_label(self, value):
        self.close_up_threshold = value
        self.headsize_value_label.configure(text=f"{int(value)}%")
        
    def update_teeth_label(self, value):
        self.teeth_sensitivity = value
        self.teeth_value_label.configure(text=f"{int(value)}%")
    ...
    def start_processing(self):
        ...
        self.teeth_slider.configure(state="disabled")
    ...
    def process_images_thread(self):
        ...
        for i, image_path in enumerate(all_files):
            ...
            if is_blank(image_path):
                ...
                facial_features = analyze_facial_features(image_path, self.teeth_sensitivity)
        ...
        self.teeth_slider.configure(state="normal")
