...
def write_exif_data(image_path, score, is_keeper=False):
    """Writes the score and an optional 5-star rating to the image's EXIF data."""
    try:
...
class App(ctk.CTk):
    ...
    def process_completed_set(self, set_data, dest_dir):
        ...
        if best_closeup:
            write_exif_data(best_closeup, best_closeup_score, is_keeper=True)
            shutil.copy(best_closeup, dest_dir)
        if best_longshot:
            write_exif_data(best_longshot, best_longshot_score, is_keeper=True)
