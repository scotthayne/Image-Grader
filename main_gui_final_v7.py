...
def write_exif_data(image_path, score, rating=None):
    """Writes the score and an optional rating by re-saving the image."""
    try:
        ...
        # Write the grade to the standard description field
        exif_dict['0th'][piexif.ImageIFD.ImageDescription] = f"PhotoGrade: {score:.2f}".encode('utf-8')
        
        # Explicitly remove other title-like tags to prevent duplication
...
class App(ctk.CTk):
    ...
    def process_completed_set(self, set_data, dest_dir):
        """Finds the best of each shot type, tags them, and copies them."""
        ...
        if best_closeup:
            self.status_label.configure(text=f"Tagging and copying {os.path.basename(best_closeup)}...")
            write_exif_data(best_closeup, best_closeup_score, rating=5)
            shutil.copy(best_closeup, dest_dir)
        if best_longshot:
            self.status_label.configure(text=f"Tagging and copying {os.path.basename(best_longshot)}...")
            write_exif_data(best_longshot, best_longshot_score, rating=5)
            shutil.copy(best_longshot, dest_dir)

    def process_images_thread(self):
        source_dir = self.source_directory
        dest_dir = os.path.join(source_dir, "keepers_rated")
        ...
        for i, image_path in enumerate(all_files):
            ...
            if is_blank(image_path):
                ...
                if quality_metrics and facial_features['face_size'] > 0:
                    ...
                    write_exif_data(image_path, total_score)
        ...
        self.status_label.configure(text=f"Processing Complete. 5-star keepers saved to '{dest_dir}'")
