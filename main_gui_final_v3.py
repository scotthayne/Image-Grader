...
def write_exif_data(image_path, score, rating=None):
    """Writes the score and an optional rating to the image's EXIF data."""
    try:
...
class App(ctk.CTk):
    ...
    def process_images_thread(self):
        ...
        keeper_files = [os.path.join(dest_dir, f) for f in os.listdir(dest_dir)]
        for keeper_path in keeper_files:
            # We don't have the score here, so we can't write it again.
            # We just add the 5-star rating.
            write_exif_data(keeper_path, 0, rating=5) # Score is a placeholder
