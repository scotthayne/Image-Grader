...
def analyze_facial_features(image_path):
    ...
    try:
        ...
        if img is None: return {'eyes_open': 0, 'smile_score': 0, 'face_size': 0, 'looking_at_camera': 0}
        ...
        with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True) as face_mesh:
            ...
            if not results.multi_face_landmarks:
                return {'eyes_open': 0, 'smile_score': 0, 'face_size': 0, 'looking_at_camera': 0}
            ...
            looking_at_camera = 1 if deviation < 0.2 else 0
            ...
            return {'eyes_open': eyes_open, 'smile_score': smile_score, 'face_size': face_size_percentage, 'looking_at_camera': looking_at_camera}
...
class App(ctk.CTk):
    ...
    def process_completed_set(self, set_data, dest_dir):
        ...
        for image_path, total_score, is_closeup, eyes_open, looking_at_camera in set_data:
            if not eyes_open or not looking_at_camera:
                continue
        ...
        for i, image_path in enumerate(all_files):
            ...
            if is_blank(image_path):
                ...
                if quality_metrics and facial_features['face_size'] > 0:
                    ...
                    total_score = facial_features['smile_score'] + eyes_score + quality_score
                    
                    write_grade_to_exif(image_path, total_score)
                    is_closeup = facial_features['face_size'] > self.close_up_threshold
                    current_set_data.append((image_path, total_score, is_closeup, facial_features['eyes_open'], facial_features['looking_at_camera']))
