...
def analyze_facial_features(image_path):
    ...
    try:
        ...
        if has_smile:
            ...
            # Define range for "white-ish" colors in HSV
            lower_white = np.array([0, 0, 200])
            upper_white = np.array([180, 50, 255])
            
            mask = cv2.inRange(hsv_smile, lower_white, upper_white)
            white_percentage = (cv2.countNonZero(mask) / (sw * sh)) * 100
            
            if white_percentage > 5: # If more than 5% of the smile area is white
                teeth_showing = 1
        ...
        for i, image_path in enumerate(all_files):
            ...
            if is_blank(image_path):
                ...
                if quality_metrics and facial_features['face_size'] > 0:
                    ...
                    teeth_score = facial_features['teeth_showing'] * 5 # New teeth bonus
