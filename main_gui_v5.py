...
def analyze_facial_features(image_path):
    """Analyzes for smiles, open eyes, and face size."""
    try:
        img = cv2.imread(image_path)
        if img is None: return {'eyes_open': 0, 'smiles': 0, 'face_size': 0}
        
        img_height, img_width, _ = img.shape
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        faces = FACE_CASCADE.detectMultiScale(gray, 1.3, 5)
        
        if len(faces) == 0: return {'eyes_open': 0, 'smiles': 0, 'face_size': 0}
        ...
        return {'eyes_open': 1 if len(eyes) >= 2 else 0, 'smiles': 1 if len(smiles) > 0 else 0, 'face_size': face_size_percentage}
    except Exception:
        return {'eyes_open': 0, 'smiles': 0, 'face_size': 0}
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.slider_label = ctk.CTkLabel(self.slider_frame, text="Close-up if face width >")
        self.slider_label.grid(row=0, column=0, padx=10, pady=10)
        self.threshold_slider = ctk.CTkSlider(self.slider_frame, from_=10, to=50, number_of_steps=40, command=self.update_threshold_label)
        self.threshold_slider.set(25)
        self.threshold_slider.grid(row=0, column=1, padx=10, pady=10, sticky="ew")
        self.threshold_value_label = ctk.CTkLabel(self.slider_frame, text="25%")
        self.threshold_value_label.grid(row=0, column=2, padx=10, pady=10)
    ...
    def process_images_thread(self):
        ...
        for set_index, image_set in enumerate(sets):
            ...
            for image_path in image_set:
                ...
                time_per_file = elapsed_time / processed_files if processed_files > 0 else 0
