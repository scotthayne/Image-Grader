...
def write_score_to_exif(image_path, score):
    ...
    try:
        ...
        # If image has no EXIF data, create it
        try:
...
class App(ctk.CTk):
    ...
    def process_images_thread(self):
        ...
        dest_dir = os.path.join(source_dir, "keepers_graded")
        ...
        for i, image_path in enumerate(all_files):
            self.status_label.configure(text=f"Processing {os.path.basename(image_path)}...")
            ...
            if quality_metrics:
                ...
                # Quality Score (40%) - Composite
                # Sharpness (normalized to ~15 points)
                sharpness_score = min(quality_metrics['sharpness'] / 100.0, 15)
                # Contrast (normalized to ~15 points)
                contrast_score = min(quality_metrics['contrast'] / 10.0, 15)
                # Brightness (10 points, peaks at 128)
                ...
                # Simple logic to copy high-scoring photos
                if total_score > 80: # Higher threshold for better selection
