...
def write_exif_data(image_path, score=None, rating=None):
    """Writes the score and/or a rating by re-saving the image."""
    try:
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # --- Target Directory for Rating Transfer ---
    ...
    def set_ui_state(self, state):
        ...
        # The transfer button is handled separately
        if state == "normal":
    ...
    def process_completed_set(self, set_data, dest_dir):
        ...
        if best_closeup:
            write_exif_data(best_closeup, score=best_closeup_score, rating=5)
            shutil.copy(best_closeup, dest_dir)
        if best_longshot:
            write_exif_data(best_longshot, score=best_longshot_score, rating=5)
    ...
    def process_images_thread(self):
        ...
        for i, image_path in enumerate(all_files):
            ...
            if is_blank(image_path):
                ...
                if quality_metrics and facial_features['face_size'] > 0:
                    ...
                    write_exif_data(image_path, score=total_score)
        ...
        self.status_label.configure(text=f"Grading Complete. Keeper folder is ready.")
        self.time_label.configure(text="You can now select a target directory and transfer ratings.")
    ...
    def transfer_ratings_thread(self):
        ...
        for i, keeper_file in enumerate(keeper_files):
            ...
            if keeper_basename in target_files:
                ...
                write_exif_data(target_path, rating=5)
                transferred_count += 1
